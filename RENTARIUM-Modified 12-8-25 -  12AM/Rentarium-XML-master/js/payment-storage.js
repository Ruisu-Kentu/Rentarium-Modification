...
const PaymentStorage = {
  ...
  getCurrentTenant() {
    ...
    // Find the current tenant by username
    const tenant = tenants.find(t => t.username === user.username);
    
    if (!tenant) return null;

    // Return standardized tenant object
    return {
      id: tenant.username, // Use username as unique ID
      name: tenant.name,
  ...
  createPayment(paymentData) {
    ...
    // CRITICAL FIX: Determine payment type correctly
    let paymentType = 'Monthly Rent'; // Default
    
    console.log("🎯 CREATE PAYMENT - paymentData received:", paymentData);
    
    // Priority 1: Use paymentData.paymentType if provided
    if (paymentData.paymentType && paymentData.paymentType.trim() !== '') {
        paymentType = paymentData.paymentType;
        console.log("🎯 Using provided paymentType:", paymentType);
    }
    // Priority 2: If billDetails exists, it's Utility Bills
    else if (paymentData.billDetails && paymentData.billDetails.trim() !== '') {
        paymentType = 'Utility Bills';
        console.log("🎯 Setting to Utility Bills because billDetails exists");
    }
    // Priority 3: Check if amount is not the usual rent amount
    else if (paymentData.amount !== tenant.monthlyRent) {
        paymentType = 'Utility Bills';
        console.log("🎯 Setting to Utility Bills because amount differs from monthly rent");
    }
    ...
    const payment = {
      ...
      paymentType: paymentType, // Use the determined type
    ...
    console.log("💾 Saving payment with type:", payment.paymentType);
